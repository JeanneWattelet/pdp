...
public class GrapheTrajet implements java.io.Serializable{
	...
	public void ajouterSommets(List<Ligne> LignesTrajet) {
		String sommet;
		Horaire h;
	...
	private void ajouterAretesDeTransport(List<Ligne> LignesTrajet){
		...
		for(Ligne l: LignesTrajet) {//On regarde les lignes successivement
			...
			for(Trajet t: tr) {//Pour chaque trajets de ces lignes
				tmp = t.getArrets();
				for(Station stationA: tmp.keySet()){//On regarde chaque station par lequel le trajet passe, qui pourraient �tre une source d'un arc
					hA = tmp.get(stationA);	
